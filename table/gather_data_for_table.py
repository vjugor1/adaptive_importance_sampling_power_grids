...
def write_planes_gather_data(pi_over, net_name_list):
    for (net, name) in net_name_list:
        pp.rundcpp(net);
        ppc = net["_ppc"];
        ...
        np.savetxt(name + 'pi' + str(pi_over) + "_Omega" + ".txt", x_An, fmt="%s")
        np.savetxt(name + 'pi' + str(pi_over) + "_Tau" + ".txt", x_bn, fmt="%s")
        ...
        ### compute expectation and std final and history
        aloe_exp_inf = p_up*np.sum(1./np.sum(x_bn <= smp.T[:],axis=1))/nsmp_ + prb_rmd; 
        aloe_std_inf = p_up*math.sqrt(2*len(_hpl))/math.sqrt(nsmp_); # indeed len(_hpl) instead of 2*m in the Thrm
        


        print("ALOE inf (exp, std)", (aloe_exp_inf, aloe_std_inf))
        ...
        output_dict = {}
        output_dict['MD-Var'] = {}
        output_dict['MD-Var']['exp'] = md_exp
        output_dict['MD-Var']['std'] = md_std
        output_dict['MD-KL'] = {}
        output_dict['MD-KL']['exp'] = kl_exp
        output_dict['MD-KL']['std'] = kl_std
        output_dict['ALOE'] = {}
        output_dict['ALOE']['exp'] = aloe_exp
        output_dict['ALOE']['std'] = np.std(aloe_exp_history)
        output_dict['ALOE-inf'] = {}
        output_dict['ALOE-inf']['exp'] = aloe_exp_inf
        output_dict['ALOE-inf']['std'] = aloe_std_inf
        with open(name + '_pi' + str(pi_over) + '_table.json', 'w+') as fp:
