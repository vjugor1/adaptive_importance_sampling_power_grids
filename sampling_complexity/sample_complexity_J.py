...
import time 
def write_results_polygon(J = 360, tau = 3):
    ...
    #mc_std_history
    aloe_std_history = np.array([np.std(aloe_exp_history[:(i+1)]) for i in range(nsmp)])
    #md_std_history
    #kl_std_history
    histories = {'MC_exp': [float(a) for a in mc_exp_history], 'MD_exp': [float(a) for a in md_exp_history], 'KL_exp': [float(a) for a in kl_exp_history], 'ALOE_exp': [float(a) for a in aloe_exp_history],
                'MC_std': [float(a) for a in mc_std_history], 'MD_std': [float(a) for a in md_std_history], 'KL_std': [float(a) for a in kl_std_history], 'ALOE_std': [float(a) for a in aloe_std_history]}
    with open('J_'+str(J) + '_.json', 'w+') as fp:
        json.dump(histories, fp)
        
Js = [60, 120, 180, 240, 320, 360, 540, 720, 1080]
for J in Js:
    write_results_polygon(J = J)
