...
def gen_data(nsmp=1000, save_for_R=True):
    ...
    np.savetxt("Regular_Omega" + ".txt", x_An, fmt="%s")
    np.savetxt("Regular_Tau" + ".txt", x_bn, fmt="%s")
    ...
    return mc_exp, md_exp, kl_exp, aloe_exp
...
save = True
for i in range(100):
    mc, md, kl, aloe = gen_data(nsmp=1000, save_for_R=save)
    mcs.append(mc / math.exp(- 6**2 / 2))
    mds.append(md / math.exp(- 6**2 / 2))
    kls.append(kl / math.exp(- 6**2 / 2))
    aloes.append(aloe / math.exp(- 6**2 / 2))
    save = False
output = {}
output['MD-Var'] = mds
output['MC'] = mcs
